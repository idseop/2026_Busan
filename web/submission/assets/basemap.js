...
window.BUSAN_CREATE_MAP=({element,features,onSelect})=>{
 ...
 function connect(){
  ...
  const probe=document.createElement('canvas');let glAvailable=false;try{const gl=probe.getContext('webgl2')||probe.getContext('webgl');glAvailable=Boolean(gl);gl?.getExtension('WEBGL_lose_context')?.loseContext();}catch(e){}
  if(!glAvailable){contextError='webgl';message.textContent='기본 지도로 표시합니다. 지역 결과와 시설은 이용할 수 있습니다.';status.hidden=false;return;}
  try{
