...
window.BUSAN_CREATE_MAP=({element,features,onSelect})=>{
 ...
 function districtFocus(){clearFocus();}
 ...
 function themeUpdate(){
  ...
  controls.querySelectorAll('[data-theme]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.theme===theme)));labels();
 }

 function setMode(value){mode=value;controls.querySelectorAll('[data-mode]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.mode===mode)));if(ready)map.setLayoutProperty('explicit-height-buildings','visibility',mode==='3d'?'visible':'none');map.easeTo({pitch:mode==='3d'?52:0,bearing:mode==='2d'?0:map.getBearing(),duration:500});}
