(() => {
 ...
 function renderCase(c,r,t,node){
  const item=knownCase(r,t),h=t.holdout||{},uid=key(r)+'|'+t.subtype,steps=['신고 패턴','지역 여건','현재 대응','보완 제안'];let current=0;
  ...
  node.innerHTML=`<div class="evidence-result" data-evidence-key="${esc(uid)}"><p class="evidence-period">2020–2024 · 정상 처리·업무성 기록 제외</p><p class="evidence-note">연도 필터와 별도로 5년을 함께 분석</p><article class="evidence-verdict"><span>${esc(r.rawDong)} · ${esc(labels[t.subtype]||t.subtype)}</span><h4>${item?'이 지역에서 검토할 보완 방향':'신고·지역 여건을 먼저 확인할 지역'}</h4><p>${esc(item?item.proposal:'연결된 운영 근거가 없어 보완안은 아직 정하지 않았습니다. 아래에서 신고 패턴과 지역 여건을 확인할 수 있습니다.')}</p><button type="button" data-evidence-verdict>${item?'제안과 근거 확인 →':'지역 여건 확인 →'}</button></article><nav class="evidence-steps" aria-label="지역 결과 4단계">${steps.map((name,i)=>`<button type="button" data-evidence-step="${i+1}" aria-current="${i===0?'step':'false'}"><span>${i+1}</span>${name}</button>`).join('')}</nav><div class="evidence-paging"><button type="button" data-evidence-prev>← 이전</button><span data-evidence-progress aria-live="polite"></span><button type="button" data-evidence-next>다음 →</button></div><div data-evidence-body></div><a class="evidence-full" target="_blank" rel="noopener" href="${esc(link(r)+'&type='+encodeURIComponent(t.subtype))}">지역 상세 자료 보기 ↗</a></div>`;
  ...
  function show(step,focus=false){
   ...
   body.innerHTML=`<h4 class="evidence-step-title" tabindex="-1">${['어떤 신고가 반복되나요?','어떤 지역 여건을 고려하나요?','이미 어떤 대응을 하고 있나요?','무엇을 더 보완할 수 있나요?'][step]}</h4>`;
   if(step===0){
   ...
   if(focus){const heading=body.querySelector('.evidence-step-title');heading.focus({preventScroll:true});heading.scrollIntoView({block:'nearest',behavior:'smooth'});}
  }
  node.querySelector('[data-evidence-verdict]').onclick=()=>show(item?3:1,true);node.querySelectorAll('[data-evidence-step]').forEach(b=>b.onclick=()=>show(Number(b.dataset.evidenceStep)-1,true));node.querySelector('[data-evidence-prev]').onclick=()=>show(Math.max(0,current-1),true);node.querySelector('[data-evidence-next]').onclick=()=>show(Math.min(3,current+1),true);show(0);
 }
 function render(c){
  lastContext=c;
  ...
  root.innerHTML=`<div class="evidence-heading"><h3>우리 지역의 대응 방향</h3><button data-evidence-expand aria-pressed="false">넓게</button></div><section class="evidence-guide" aria-label="대응 방향 분석 흐름"><strong>이 화면은 신고 통계에서 보완 방향까지 근거를 차례로 확인하는 곳입니다.</strong><ol><li><b>1 신고 패턴</b><span>반복 유형과 시기 확인</span></li><li><b>2 지역 여건</b><span>인구·주거 등 배경 확인</span></li><li><b>3 현재 대응</b><span>운영 중인 서비스 대조</span></li><li><b>4 보완 제안</b><span>추가 검토 방향 정리</span></li></ol><p>신고 건수만으로 위험도나 사업 우선순위를 확정하지 않습니다.</p></section>${c.selection.kind==='district'?`<label class="evidence-label">이 구·군의 지역<select data-evidence-region>${matches.map(x=>`<option value="${esc(x.rawDong)}" ${x===r?'selected':''}>${esc(x.rawDong)}</option>`).join('')}</select></label>`:''}<label class="evidence-label">심층 분석 유형<select data-evidence-domain></select></label><div data-evidence-result></div>`;
  if(c.tab==='services')c.content.replaceChildren(root);else{c.content.querySelector('.final-case-links')?.remove();c.content.querySelector('.open-regional-evidence')?.remove();return;}
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  const tab=document.querySelector('[data-tab="services"]');if(tab)tab.textContent='대응 방향';
