    ...
    try {
      ...
      const content = rows => {
        ...
        rows.forEach(row=>{
          ...
          add('출동가능 인원', fmt(row.personnel)+'명');
          add('출동가능 차량', fmt(row.vehicles)+'대');
          if(row.vehicleTypes)add('차량 유형', row.vehicleTypes);
          item.append(name,type,address,stats);div.append(item);
        });
        const note=document.createElement('small');note.textContent='출동가능 인원·차량은 제공 CSV를 관서명·주소로 연결한 값입니다. 기준일 미기재. 신고 발생 위치가 아닙니다.';div.append(note);return div;
      };
      groups.forEach(rows=>{
        const row=rows[0];
