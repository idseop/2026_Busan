...
with sync_playwright() as pw:
 ...
 for w,h in [(1920,1080),(1600,900),(1366,768)]:
  ...
  assert p.locator('section').count()==10
  assert p.locator('figure img').count()==3
  ...
  checks.append({'viewport':[w,h],'reportSections':10,'figures':3,'overflow':False})
 for scope in ['A','B','C']:
 ...
 for path in [O/'index.html',F/'explorer.html',F/'쟁점해결.html']:
  ...
  for a in p.locator('a').all():
   ...
   if h and not h.startswith(('http','#')):assert (path.parent/unquote(h.split('#')[0])).exists(),h
